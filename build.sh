...
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]] && [[ ! $(echo $TARGET | grep 'test') ]]; then echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin; fi
...
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]] && [[ ! $(echo $TARGET | grep 'test') ]]; then docker push $DOCKER_USERNAME/ros2:arm64v8-$TARGET; fi
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]] && [[ ! $(echo $TARGET | grep 'test') ]]; then docker push $DOCKER_USERNAME/ros2:x64-$TARGET; fi
docker images
...
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]] && [[ ! $(echo $TARGET | grep 'test') ]]; then docker manifest push $DOCKER_USERNAME/ros2:$TARGET; fi
